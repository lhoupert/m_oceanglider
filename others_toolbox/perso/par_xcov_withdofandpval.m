function [xcf,lags,pval,dofy1,dofy2] = par_xcov_withdofandpval(y1,y2,maxlag)
% function [xcf,lags,pval,dofy1,dofy2] = par_xcov_withdofandpval(y1,y2,maxlag)
...
py1 = parallel.pool.Constant(y1(:));
py2 = parallel.pool.Constant(y2(:));
%py1.Value = y1(:);
%py2.Value = y2(:);
parfor cc=1:length(lags)
    y1b = py1.Value;
    y2b = py2.Value;
