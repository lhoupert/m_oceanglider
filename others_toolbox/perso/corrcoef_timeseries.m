function [r,p,rlo,rup] = corrcoef_timeseries(x,varargin)
%CORRCOEF Correlation coefficients with Degree of freedom adjust from
%Integral Timescales
...
if ~isempty(varargin) && isnumeric(varargin{1})
   ...
   [~,dof1] = decorrelation3(1:length(x),x,0);  
   [~,dof2] = decorrelation3(1:length(y),y,0);  
   dof = min(dof1,dof2);
   x = [x(:) y(:)];
