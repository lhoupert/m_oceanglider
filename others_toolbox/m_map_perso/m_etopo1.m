...
PATHNAME='/home/loic/Boulot/function_MATLAB/bathy/';   % Be sure to end the path with a "/" or
...
efid=fopen([PATHNAME 'GOL_base.dat'],'r','b'); % in big-endian format
...
if draw_map,
  ...
  blat=floor(MAP_VAR_LIST.lats(1)*60);
  tlat=ceil(MAP_VAR_LIST.lats(2)*60);
  llong=floor(MAP_VAR_LIST.longs(1)*60);
  rlong=ceil(MAP_VAR_LIST.longs(2)*60);
  ...
  blat=floor(varargin{1}(3)*60);
  tlat=ceil(varargin{1}(4)*60);
  llong=floor(varargin{1}(1)*60);
  rlong=ceil(varargin{1}(2)*60);
  lngdec=1;
  latdec=1;

end;

lgs=[llong:lngdec:rlong]/60;
lts=fliplr([blat:latdec:tlat]/60);

if rlong>(180*60), rlong=rlong-360*60; llong=llong-360*60; end;
if llong<-(180*60), rlong=rlong+360*60; llong=llong+360*60; end;

eaxes=[llong+180*60 rlong+180*60 90*60-blat 90*60-tlat];
...
if (eaxes(2)>10800 ),   % Read it in in 2 pieces!
  ...
  for ii=[1:nlat],
   fseek(efid,(ii-1+eaxes(4))*((360*60+1)*2),'bof');
   values(ii,nlng+[-nlgr:-1]+1)=fread(efid,[1 nlgr],'int16');
   fseek(efid,(ii-1+eaxes(4))*((360*60+1)*2)+eaxes(1)*2,'bof');
