function [vbdbias, w_rms_final] = dd_regress_vbd(...
  handles, c_vbd_ref, diveCycleNumbers, settings)
%
%   handles - Handles to GUI
%   dataFoilder - Path to datafiles
%   c_vbd_ref - User specified c_vbd constant - NOT USED!
%   diveCycleNumbers - List of selected dives
%   settings - Settings struct
%
% Returns:
%   vbdbias - Computed VBD bias
%   w_rms_final - Computed w_rms value
%   
doPlots = 1;
dataFolder = settings.pathname;
...
% KUTI
hWaitbar = waitbar(0, sprintf('Reading dive data...'));
...
% KUTI - the dives were passed in as a parameter
...
% KUTI - not supported - mass_comp not available
if 0
...
% KUTI - get id from settings param
...
for ir = dives'
	...
	id_dive = sprintf('%03d%04d',settings.seagliderID, ir); %diveNumberList(i));
	...
	if (isfield(results,'processing_error') || isfield(results,'skipped_profile'))
        ...
        % KUTI
        strResults = sprintf('Unable to include data for dive %d', ir);
	...
	fprintf(1,'p%s\n',id_dive);
    % KUTI
	strResults = sprintf('p%s', id_dive);
	updateResults(handles, strResults);
...
% KUTI
waitbar(0, hWaitbar, 'Calculating...');
...
if (mp == 0)
    ...
    % KUTI
    strResults = sprintf('No data available!');
    updateResults(handles, strResults);
...
% KUTI
strResults = sprintf('Initial parameter values:');
updateResults(handles, strResults);

fprintf(1,'$C_VBD,%4d\n', round(first_c_vbd));
% KUTI
strResults = sprintf('$C_VBD,%4d', round(first_c_vbd));
updateResults(handles, strResults);

fprintf(1,'Given vbdbias = %5.4fcc and volmax = %6.1fcc\n', vbdbias_0,volmax);
% KUTI
strResults = sprintf('Given vbdbias = %5.4fcc and volmax = %6.1fcc\n', vbdbias_0,volmax);
updateResults(handles, strResults);

fprintf(1,'Hydrodynamic parameters: hd_a = %6.5e; hd_b = %6.5e;hd_c = %6.5e\n', hd_a, hd_b, hd_c);
% KUTI
strResults = sprintf('Hydrodynamic parameters: hd_a = %6.5e; hd_b = %6.5e;hd_c = %6.5e\n', hd_a, hd_b, hd_c);
updateResults(handles, strResults);

fprintf(1,'Hull parameters: abs_compress = %5.4e; therm_expan  = %5.4e\n', abs_compress, therm_expan);
% KUTI
strResults = sprintf('Hull parameters: abs_compress = %5.4e; therm_expan  = %5.4e\n', abs_compress, therm_expan);
updateResults(handles, strResults);
...
% KUTI
strResults = sprintf('Initial RMS diff. between model and actual w is %5.4f cm/s.\n', w_rms_initial);
updateResults(handles, strResults);
...
% KUTI
strResults = sprintf('Determining vbdbias...');
updateResults(handles, strResults);
...
% KUTI
strResults = sprintf('Found vbdbias: %5.4fcc\n',vbdbias);
updateResults(handles, strResults);
...
if (fit_abc)
    ...
    % KUTI
	strResults = sprintf('Fitting hydrodynamic parameters...');
	updateResults(handles, strResults);  
    waitbar(0.4, hWaitbar, 'Fitting hydrodynamic parameters, please wait...');

    %if fitting a.b.&c use more evaluations of misfit
	opt = optimset('maxfunevals',2000,'display',display_opt,'tolfun',1e-3);
fprintf(1, 'after optimset\n');
...
fprintf(1, 'after fminsearch\n');
	vbdbias = x(1); a = x(2); b = x(3); c = x(4);
	%DEBUG fprintf(1,'Found vbdbias = %5.4fcc and hydrodynamic parameters: a = %6.5e; b = %6.5e;c = %6.5e\n', vbdbias, a, b, c);
    waitbar(0.6, hWaitbar, 'Fitting hydrodynamic parameters...');
else
	fprintf(1,'Skipping fit of hydrodynamic parameters due to insufficient dives...\n');
    % KUTI
	strResults = sprintf('Skipping fit of hydrodynamic parameters due to insufficient dives...');
	updateResults(handles, strResults);
end

waitbar(0.8, hWaitbar, 'Calculating...');
if (fit_exp_comp) 
	fprintf(1,'C\n');
    % KUTI
    waitbar(0.8, hWaitbar, 'Determining compressibility and thermal expansion factors for hull and fairing..');
	strResults = sprintf('Determining compressibility and thermal expansion factors for hull and fairing..');
	updateResults(handles, strResults);
...
waitbar(0.9, hWaitbar, 'Calculating...');
...
if (w_rms_fit > good_w_rms_fit)
    ...
    % KUTI
    strResults = sprintf('NOTE: Regression did not yield a good set of parameters.');
    updateResults(handles, strResults);
	...
	if (length(better_dives) > 4)
        ...
        % KUTI
        strResults = sprintf('Try the following set of dives instead: %s\n',succinct_elts(better_dives));
        updateResults(handles, strResults);
	...
	if (~isempty(ratty_dives))
        ...
        % KUTI
        strResults = sprintf('NOTE: While the overall RMS was good, the following dives were not terribly good: %s.\n',succinct_elts(ratty_dives));
        updateResults(handles, strResults);
	end
	fprintf(1,'Recommended $C_VBD,%4d\n', fix(implied_c_vbd_volmax));
    % KUTI
    strResults = sprintf('Recommended $C_VBD,%4d\n', fix(implied_c_vbd_volmax));
    updateResults(handles, strResults);
    ...
    % KUTI
	strResults = sprintf('Add the following lines to the bottom of sg_calib_constants.m:\n');
    updateResults(handles, strResults);
	if (vbdbias_0) % they set it in sg_calib_constants.m; should be removed
		strResults = sprintf('NOTE: Ensure vbdbias setting is removed or is set to 0!\n');
        updateResults(handles, strResults);
	end
	strResults = sprintf('%% %s RMS=%5.4f cm/s Dives: %s\n',timelabel,w_rms_fit,dives);
    updateResults(handles, strResults);

end

waitbar(0.10, hWaitbar, 'Preparing VBD plots (this may take a minute)...');
fprintf(1,'volmax = %6.1f;\n',implied_volmax);
% KUTI
strResults = sprintf('volmax = %6.1f;\n',implied_volmax);
updateResults(handles, strResults);
...
if (fit_abc == 1)
	...
	fprintf(1,'hd_c = %6.5e;\n', c);
    % KUTI
    strResults = sprintf('hd_a = %6.5e;\n', a);
    updateResults(handles, strResults);
    strResults = sprintf('hd_b = %6.5e;\n', b);
    updateResults(handles, strResults);
    strResults = sprintf('hd_c = %6.5e;\n', c);
    updateResults(handles, strResults);
end

waitbar(0.20, hWaitbar, 'Preparing VBD plots (this may take a minute)...');

if (fit_exp_comp)
	fprintf(1,'abs_compress = %5.4e;\n', abs_compress_opt);
	fprintf(1,'therm_expan  = %5.4e;\n', therm_expan_opt);
    % KUTI
    strResults = sprintf('abs_compress = %5.4e;\n', abs_compress_opt);
    updateResults(handles, strResults);
    strResults = sprintf('therm_expan  = %5.4e;\n', therm_expan_opt);
    updateResults(handles, strResults);
end

waitbar(0.30, hWaitbar, 'Preparing VBD plots (this may take a minute)...');

% KUTI
%fprintf(1, '\nNOTE - please wait for all four plots to be displayed...\n\n');
%strResults = sprintf('\nNOTE - please wait for all four plots to be displayed...\n\n');
%updateResults(handles, strResults);
...
if doPlots
    ...
    % KUTI - makes 'Close Plots' button work.
    %figure;
    hFigPlot = figure('Name','Regress_VBD_1');
    set(hFigPlot, 'UserData', 'Plot_1');
    ...
    % KUTI - makes 'Close Plots' button work.
    %figure;
    hFigPlot = figure('Name','Regress_VBD_2');
    set(hFigPlot, 'UserData', 'Plot_2');
    ...
    waitbar(0.40, hWaitbar, 'Preparing VBD plots (this may take a minute)...');
    ...
    waitbar(0.50, hWaitbar, 'Preparing VBD plots (this may take a minute)...');
    ...
    % KUTI - makes 'Close Plots' button work.
    %figure;
    hFigPlot = figure('Name','Regress_VBD_3');
    set(hFigPlot, 'UserData', 'Plot_3');
    ...
    % KUTI - makes 'Close Plots' button work.
    %figure
    hFigPlot = figure('Name','Regress_VBD_4');
    set(hFigPlot, 'UserData', 'Plot_4');

    waitbar(0.60, hWaitbar, 'Preparing VBD plots (this may take a minute)...');
    ...
    waitbar(0.99, hWaitbar, 'Done preparing VBD plots...');

end % doPlots (otherwis we're just performing regression)
